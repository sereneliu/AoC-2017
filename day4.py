...
passphrase_string = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
passphrase_list = [[str(word) for word in phrase.split()] for phrase in passphrase_string.splitlines()]

def find_valid_phrases(some_list):
    valid_phrases = 0
...
print find_valid_phrases(passphrase_list)
...
def no_anagrams(some_list):
    valid_phrases = 0
    for phrase in some_list:
        if len(phrase) == len(set("".join(sorted(word)) for word in phrase)):
            valid_phrases += 1
    return valid_phrases

print no_anagrams(passphrase_list)